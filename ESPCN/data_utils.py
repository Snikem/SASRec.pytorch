...
def generate_dataset(data_type, upscale_factor, dataset_path):
    images_name = [x for x in listdir(dataset_path) if is_image_file(x)]
    if data_type == "train":
        images_name = images_name[:-500]
    else:
        images_name = images_name[-500:]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_path', default='', type=str, help="path to dataset of images")
    parser.add_argument('--upscale_factor', default=3, type=int, help='super resolution upscale factor')
    opt = parser.parse_args()
    DATASET_PATH = opt.dataset_path
    UPSCALE_FACTOR = opt.upscale_factor

    generate_dataset(data_type='train', upscale_factor=UPSCALE_FACTOR, dataset_path=DATASET_PATH)
    generate_dataset(data_type='val', upscale_factor=UPSCALE_FACTOR, dataset_path=DATASET_PATH)
